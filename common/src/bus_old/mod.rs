...
impl BusData {
    fn new(client: Client) -> Self {
        Self {
            shutdown: false,
            presence: Presence::new(client.handle().clone()),
            metadata: Metadata::new(client.handle().clone()),
            client,
        }
    }
...
impl Transport {
    ...
    async fn run(mut self) {
        ...
        let client_event_loop = self.data.client.run();
        tokio::pin!(client_event_loop);

        loop {
            select! {
                _ = &mut client_event_loop => {
                    error!("Got unexpected event loop end");
                    break;
                }
        ...
        std::mem::drop(client_event_loop);
