use std::{collections::HashMap, fmt, str, num::ParseIntError};
use regex::Regex;
...
impl str::FromStr for Type {
    ...
    fn from_str(input: &str) -> Result<Self, Self::Err> {
        ...
        let matchs = PARSER.captures(input).ok_or_else(|| TypeParseError::new(input, TypeParseErrorReason::InvalidType))?;
        ...
        match matchs.len() {
            ...
            _ => {
                return Err(TypeParseError::new(input, TypeParseErrorReason::BadMatchLen));
            }
        }

        match base_type {
            "range" => {
                let matchs = RANGE_PARSER.captures(args).ok_or_else(|| TypeParseError::new(input, TypeParseErrorReason::BadArgs))?;
                if matchs.len() != 3 {
                    return Err(TypeParseError::new(input, TypeParseErrorReason::BadArgs));
                }

                let min = matchs[1].parse::<i64>().map_err(|err| TypeParseError::new(input, TypeParseErrorReason::BadValue(err)))?;
                let max = matchs[2].parse::<i64>().map_err(|err| TypeParseError::new(input, TypeParseErrorReason::BadValue(err)))?;
            ...
            "text" => {
                if args != "" {
                    return Err(TypeParseError::new(input, TypeParseErrorReason::UnexpectedArgs));
                }
                return Ok(Type::Text);
            },
            "float" => {
            ...
            "enum" => {
                let matchs = ENUM_PARSER.captures(args).ok_or_else(|| TypeParseError::new(input, TypeParseErrorReason::BadArgs))?;
                if matchs.len() != 2 {
                    return Err(TypeParseError::new(input, TypeParseErrorReason::BadArgs));
                }
            ...
            _ => {
                return Err(TypeParseError::new(input, TypeParseErrorReason::UnknownType));
            }
        }
    }
}
...
impl TypeParseError {
    pub fn new(input: &str, reason: TypeParseErrorReason) -> Self {
        TypeParseError { input: input.into(), reason }
    }
}
